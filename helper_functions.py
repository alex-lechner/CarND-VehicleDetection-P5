...
def convert_color(img, color_space='YCrCb'):
    if color_space.upper() == 'HSV':
        ...
        raise ValueError(
            "Error: Please define \"RGB\", \"HSV\", \"LUV\", \"HLS\", \"YUV\" or \"YCrCb\" as color space!")
...
def get_heatmap_img(img, box_list, threshold=1):
...
def read_in_images(folder):
    ...
    for filename in glob.glob(folder):
        ...
        if 'non-vehicles/' in folder:
            not_cars.append(image)
        elif 'vehicles/' in folder:
            cars.append(image)
        else:
            raise ValueError('No such folder found: ' + folder)
    if 'non-vehicles/' in folder:
        return not_cars
    elif 'vehicles/' in folder:
        return cars
